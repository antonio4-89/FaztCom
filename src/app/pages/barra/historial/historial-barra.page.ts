...
export class HistorialBarraPage implements OnInit, OnDestroy {
  comandas: Comanda[] = [];
  recentListo: Comanda[] = [];
  loading = false;
  ...
  ngOnInit() {
    this.loading = true;
    this.api.get<Comanda[]>('/comandas?destino=barra&today=true').subscribe({
      next: d => {
    ...
    this.subs.push(
      this.socket.onComandaActualizada().subscribe((c: Comanda) => {
        if (c.destino === 'barra' && c.status === 'listo') {
