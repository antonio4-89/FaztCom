...
@Component({
  ...
  templateUrl: 'menu-barra.page.html',
  styleUrls: ['menu-barra.page.scss'],
  standalone: false
})
export class MenuBarraPage implements OnInit, OnDestroy {
  menu: MenuGroup[] = [];
  loading = false;
  ...
  load() {
    ...
    this.svc.getMenu().subscribe({
      next: m => { this.menu = m['bebida'] || []; this.loading = false; },
